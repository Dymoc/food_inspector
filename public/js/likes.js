...
function animationLikes(){
    ...
    function init() {
		...
		/* Icon 11 */
        if(items[index].querySelector('.organik-icon-visibility')){
            items[index].querySelector('.organik-icon-visibility').parentNode.style.zIndex="9999"
        }
...
function animationLikesForShow(){
    'use strict';
    ...
    // grid items:
    var items = [].slice.call(document.querySelectorAll('.naming_recipe'));
			...
			tweens : [
				...
				new mojs.Tween({
					...
					onUpdate: function(progress) {
						...
						var colorProgress = opacityCurve11(progress);
						items[index].querySelector('.like').style.color = colorProgress >= 1 ? '#CC397B' : '#60BE74';
					}
				})
			],
			onUnCheck : function() {
				items[index].querySelector('.like').style.color = '#60BE74';	
			}
		});
		/* Icon 11 */
